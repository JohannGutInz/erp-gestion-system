...
const DayContent = ({ date, ordersForDay }) => {
...
export const CalendarView = ({ getOrdersForDate, serviceType, handleEditOrder, handleDeleteOrder, updateOrderStatus, sellers }) => {
  const [month, setMonth] = useState(new Date());
  const [selectedDay, setSelectedDay] = useState(null);

  const ordersByDay = useMemo(() => {
    const allOrdersInView = getOrdersForDate({ allForMonth: true, serviceType });
    const map = new Map();
    allOrdersInView.forEach(order => {
        if (order.date) {
            const dateObj = new Date(order.date);
            const dateStr = new Date(Date.UTC(dateObj.getUTCFullYear(), dateObj.getUTCMonth(), dateObj.getUTCDate())).toISOString().split('T')[0];
            if (!map.has(dateStr)) {
                map.set(dateStr, []);
            }
    ...
    <div className="relative">
      ...
      >
        <Calendar
            ...
            components={{
              DayContent: (props) => <DayContent {...props} ordersForDay={getOrdersForDay(props.date)} />
            }}
            modifiers={modifiers}
